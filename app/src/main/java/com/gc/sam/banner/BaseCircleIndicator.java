...
class BaseCircleIndicator extends LinearLayout {
    ...
    protected int mSelectedIndicatorWidth = -1;
    protected int mSelectedIndicatorHeight = -1;
    ...
    private void init(Context context, AttributeSet attrs) {
        IndicatorConfig config = handleTypedArray(context, attrs);
        initialize(config);
    ...
    private IndicatorConfig handleTypedArray(Context context, AttributeSet attrs) {
        ...
        config.animatorResId = typedArray.getResourceId(R.styleable.BaseCircleIndicator_ci_animator,
                R.anim.widget_indicator_scale_with_alpha);
        ...
        config.backgroundResId =
                typedArray.getResourceId(R.styleable.BaseCircleIndicator_ci_drawable,
                        R.drawable.widget_indicator_white_radius_selected);
        ...
        config.selectedWidth = typedArray.getDimensionPixelSize(R.styleable.BaseCircleIndicator_ci_selected_width, -1);
        config.selectedHeight = typedArray.getDimensionPixelSize(R.styleable.BaseCircleIndicator_ci_selected_height, -1);
    ...
    public void initialize(IndicatorConfig config) {
        ...
        mSelectedIndicatorWidth = (config.selectedWidth < 0) ? miniSize : config.selectedWidth;
        mSelectedIndicatorHeight = (config.selectedHeight < 0) ? miniSize : config.selectedHeight;
        ...
        mIndicatorBackgroundResId =
                (config.backgroundResId == 0) ? R.drawable.widget_indicator_white_radius_normal :
                        config.backgroundResId;
    ...
    public void createIndicators(int count, int currentPosition) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (currentPosition == i) {
                ...
                params.width = mSelectedIndicatorWidth;
                params.height = mSelectedIndicatorHeight;
    ...
    public void initSelectedView(int position) {
        View selectedIndicator = getChildAt(position);
        if (selectedIndicator != null) {
            LayoutParams params = (LayoutParams) selectedIndicator.getLayoutParams();
            params.width = mSelectedIndicatorWidth;
            params.height = mSelectedIndicatorHeight;
            selectedIndicator.setLayoutParams(params);
    ...
    public void animatePageSelected(int position) {
        ...
        if (mLastPosition >= 0 && (currentIndicator = getChildAt(mLastPosition)) != null) {
            LayoutParams params = (LayoutParams) currentIndicator.getLayoutParams();
            params.width = mIndicatorWidth;
            params.height = mIndicatorHeight;
    ...
    class IndicatorConfig {
        ...
        @AnimatorRes
        int animatorResId = R.anim.widget_indicator_scale_with_alpha;
        @AnimatorRes
        int animatorReverseResId = 0;
        @DrawableRes
        int backgroundResId = R.drawable.widget_indicator_white_radius_selected;
        @DrawableRes
        int unselectedBackgroundId = R.drawable.widget_indicator_white_radius_normal;
